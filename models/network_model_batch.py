...
class Networkn(nn.Module):
    def __init__(self, nb, downsample , kerneln, nc,in_nc=1,out_nc=1 , act_mode='BR'):
        ...
        m_head = B.conv1(in_nc*downsample, nc, kerneln, padding=kerneln//2, mode='C'+act_mode[-1], bias=bias)
        m_body = [B.conv1(nc, nc, kerneln, padding=kerneln//2, mode='C'+act_mode, bias=bias) for _ in range(nb-2)]
        m_tail = B.conv1(nc, out_nc*downsample, kerneln, padding=kerneln//2, mode='C', bias=bias)
